...
validateSig verKeyHashingFn now sigs ctx =
    State.runState (traverse (exceptions . validate) sigs) ctx
  where
    ...
    validate sig@Sig { sigId,
      ...
      PoolValidationCtx mNextEpoch pools ocertCounters <- State.get
